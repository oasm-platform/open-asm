...
func (c *workersServiceClient) DownloadTools(ctx context.Context, in *DownloadToolsRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[DownloadToolsResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &WorkersService_ServiceDesc.Streams[1], WorkersService_DownloadTools_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[DownloadToolsRequest, DownloadToolsResponse]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
func _WorkersService_DownloadTools_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(DownloadToolsRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(WorkersServiceServer).DownloadTools(m, &grpc.GenericServerStream[DownloadToolsRequest, DownloadToolsResponse]{ServerStream: stream})
}
...
var WorkersService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "DownloadTools",
			Handler:       _WorkersService_DownloadTools_Handler,
			ServerStreams: true,
		},
	},
