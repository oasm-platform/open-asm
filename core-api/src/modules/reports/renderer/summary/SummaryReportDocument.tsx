...
/** Section 01: Executive Summary — key metrics from current data */
  ...
  return (
    <View style={styles.section}>
      ...
      <View style={[styles.row, { marginBottom: 8 }]}>
        <MetricCard label="Total Targets" value={w.totalTargets} />
        <MetricCard label="Total Assets" value={w.totalAssets} />
        <MetricCard label="Total Services" value={w.totalServices} />
        <MetricCard label="Security Score" value={w.securityScore} />
      </View>
      <View style={styles.row}>
        <MetricCard label="Total Vulnerabilities" value={w.activeVulns} />
        <MetricCard label="Critical" value={w.criticalVulns} critical />
        <MetricCard label="High" value={w.highVulns} />
        <MetricCard label="Medium" value={w.mediumVulns} />
      </View>
...
const SeverityOverview: React.FC<{ data: ReportData }> = ({ data }) => (
  <View style={styles.section}>
    ...
    <SeverityDistribution severityDistribution={data.riskDistribution.map((r) => ({ level: r.level, count: r.count, percent: r.percent }))} />
  </View>
);
      ...
      <View style={styles.row}>
        ...
        <View style={[styles.panel, { flex: 1 }]}>
          <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
            ...
            <Text style={{ fontSize: 7, color: '#16a34a', fontFamily: 'Inter' }}>
              ↓ {trends.trend}
            </Text>
          </View>
...
const NewDiscoveries: React.FC<{ data: ReportData }> = ({ data }) => {
  ...
  const hasAny = d.domains.length || d.ipAddresses.length || d.ports.length || d.technologies.length;
    ...
    <View style={styles.section}>
      ...
      <DiscoveriesTable
        ...
        renderCell={(key, item) => {
          ...
          if (key === 'Risk') {
            return <RiskBadge level={domain.riskLevel} />;
          }
          return '';
        }}
      />
      <DiscoveriesTable
        ...
        renderCell={(key, item) => {
          ...
          if (key === 'Risk') {
            return <RiskBadge level={ip.riskLevel} />;
          }
          return '';
        }}
      />
      <DiscoveriesTable
        ...
        renderCell={(key, item) => {
          ...
          if (key === 'Risk') {
            return <RiskBadge level={p.riskLevel} />;
          }
          return '';
        }}
      />
...
/** Section 07: Vulnerability by Target */
