...
export class Statistic extends BaseEntity {
    ...
    @ApiProperty({ description: 'Number of critical vulnerabilities', default: 0 })
    @Column({ default: 0 })
    criticalVuls: number;

    @ApiProperty({ description: 'Number of high severity vulnerabilities', default: 0 })
    @Column({ default: 0 })
    highVuls: number;

    @ApiProperty({ description: 'Number of medium severity vulnerabilities', default: 0 })
    @Column({ default: 0 })
    mediumVuls: number;

    @ApiProperty({ description: 'Number of low severity vulnerabilities', default: 0 })
    @Column({ default: 0 })
    lowVuls: number;

    @ApiProperty({ description: 'Number of info severity vulnerabilities', default: 0 })
    @Column({ default: 0 })
    infoVuls: number;
