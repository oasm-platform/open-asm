...
export class AssetGroupWorkflowSubscriber implements EntitySubscriberInterface<AssetGroupWorkflow> {
    ...
    beforeInsert(event: InsertEvent<AssetGroupWorkflow>) {
        if (event.entity) {
            this.logger.log(`AssetGroupWorkflow insert event triggered for entity ID: ${event.entity.id}`);
        }
    ...
    beforeRemove(event: RemoveEvent<AssetGroupWorkflow>) {
        if (event.entity) {
            this.logger.log(`AssetGroupWorkflow remove event triggered for entity ID: ${event.entity.id}`);
        }
        // You can access the entity through event.entity
        // Example:
