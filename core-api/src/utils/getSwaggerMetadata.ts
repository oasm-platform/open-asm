...
export default function getSwaggerMetadata<T extends object>(
  target: new (...args: unknown[]) => T,
): SwaggerMetadataResponse<T> {
    ...
    if (meta !== undefined) {
      ...
      if (metaObj.type) {
        if (typeof metaObj.type === 'function') {
          typeName =
            typeof metaObj.type.name === 'string'
              ? metaObj.type.name.toLowerCase()
              : undefined;
          ...
          typeName =
            typeof objType.name === 'string'
              ? objType.name.toLowerCase()
              : typeof metaObj.type;
        } else {
          typeName =
            typeof metaObj.type === 'string'
              ? metaObj.type.toLowerCase()
              : undefined;
        }
      }
      ...
      (result as Record<string, SwaggerPropertyMetadata>)[key] = {
        type: typeName,
        title:
          typeof metaObj.title === 'string' ||
          typeof metaObj.title === 'number' ||
          typeof metaObj.title === 'boolean'
            ? String(metaObj.title)
            : undefined,
        example:
          typeof metaObj.example === 'string' ||
          typeof metaObj.example === 'number' ||
          typeof metaObj.example === 'boolean'
            ? metaObj.example
            : undefined,
        description:
          typeof metaObj.description === 'string' ||
          typeof metaObj.description === 'number' ||
          typeof metaObj.description === 'boolean'
            ? metaObj.description
            : undefined,
      };
    }
