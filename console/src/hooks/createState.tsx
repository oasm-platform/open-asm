...
type UseGlobalStateReturn<T, R extends ReducerMap<T>> = {
    ...
    [K in keyof R]: (...args: Parameters<R[K]> extends [any, ...infer A] ? A : never) => void;
};
...
export default function createState<T>(
    ...
    return function useGlobalState() {
        ...
        const actions = (Object.entries(reducers || {}) as [keyof ReducerMap<T>, ReducerMap<T>[keyof ReducerMap<T>]][]).reduce(
            (acc, [name, fn]) => {
                acc[name] = (...args: any[]) => setState((prev) => fn(prev, ...args));
                return acc;
            },
            {} as any
        );

        return { state, setState, ...actions } as UseGlobalStateReturn<T, ReducerMap<T>>;
    };
}
