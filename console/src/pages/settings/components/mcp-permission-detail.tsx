...
export const WorkspacePermissionDetails = ({
  permission,
}: {
      ...
      <div className="flex justify-between items-center">
        ...
        >
          ...
          <span className="text-xs bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200 rounded-full px-2 py-0.5">
            {permission.value.length} Workspace
            {permission.value.length !== 1 ? 's' : ''}
          </span>
          <span className="text-xs bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200 rounded-full px-2 py-0.5">
            {permission.value.reduce(
              (total, permValue) => total + permValue.permissions.length,
              0,
            )}{' '}
            permission
            {permission.value.reduce(
        ...
        <div className="space-y-5">
          {permission.value.map((permValue, index) => {
            const workspace = workspaces.find(
              (ws) => ws.id === permValue.workspaceId,
            );
            return (
                  ...
                  {permValue.permissions.map((permission, idx) => (
                    <Tooltip>
                      ...
                      <TooltipContent className="max-w-[200px] z-50 bg-background rounded text-foreground border p-3">
                        {tools?.find((tool) => tool.name === permission)
