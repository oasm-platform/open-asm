import type { GetManyGetAssetsResponseDtoDto } from "@/services/apis/gen/queries";
...
const AssetsDiscovering = ({ domains, targetId }: AssetsDiscoveringProps) => {
  ...
  const assets: GetManyGetAssetsResponseDtoDto | undefined =
    queryClient.getQueryData(["assets", targetId]);

  // Fallback to mock data if domains is not provided
  domains = domains || assets?.data?.map((i) => i.value);
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      setCurrentDomainIndex(
        (prevIndex) => (prevIndex + 1) % (domains?.length || 0),
      );
    }, 1000);
  ...
  return (
    <div className="flex items-center justify-center top-0 my-5 relative">
      ...
      <motion.div
        ...
        style={{
          backgroundImage:
            "linear-gradient(90deg, #3b82f6, #8b5cf6, #ec4899, #10b981, #f59e0b, #3b82f6)",
      ...
      <div
        className="relative my-5 w-full h-96 overflow-hidden rounded-2xl p-8 flex flex-col items-center justify-center transition-colors duration-500"
        style={{ zIndex: 20 }}
      >
