...
export default function AssetGroupWorkflow({
    ...
    const { data: groupWorkflows, refetch: refetchWorkflows } = useAssetGroupControllerGetWorkflowsByAssetGroupsId(assetGroupId)
    ...
    const handleToolClick = async (tool: { name: string; id: string }) => {
        ...
        if (isInGroup) {
            ...
            try {
                ...
                const updatedJobs = workflow.content?.jobs?.filter(
                    job => job.run !== tool.name
                ) || [];
                ...
                if (updatedJobs.length === 0) {
                    ...
                    toast.success(`Workflow with tool ${tool.name} removed successfully!`);
                } else {
                    ...
                    toast.success(`Tool ${tool.name} removed from workflow successfully!`);
                }
            ...
            try {
                ...
                if (existingWorkflow) {
                    ...
                    toast.success(`Tool ${tool.name} added to existing workflow successfully!`);
                } else {
                    ...
                    // Create the workflow
                    const createdWorkflow = await createWorkflowMutation.mutateAsync(workflowPayload);
                    ...
                    toast.success(`Workflow created and tool ${tool.name} added successfully!`);
                }
                    ...
                    vulnerabilityTools.map((tool) => {
                        ...
                        return (
                            <div
                                ...
                                style={{
                                    position: 'relative',
                                    cursor: isAdded ? (isProcessing ? 'wait' : 'pointer') : 'pointer'
                                }}
