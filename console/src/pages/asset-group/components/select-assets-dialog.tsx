    ...
    const assetsNotInGroupColumns: ColumnDef<Asset>[] = [
        {
            ...
            header: ({ table }) => {
                ...
                const allIds = assetsNotInGroupQuery.data?.data?.map(asset => asset.id) || [];
                const allPageRowsSelected = allIds.length > 0 && allIds.every(id => selectedAssets.includes(id));
                const somePageRowsSelected = allIds.some(id => selectedAssets.includes(id));
            ...
            <DialogContent className="sm:max-w-4xl max-h-[80vh] overflow-auto">
                ...
                <div className="py-4">
                    <DataTable
                        ...
                        tableState={{
                            rowSelection: selectedAssets.reduce((acc, id) => {
                                acc[id] = true;
                                return acc;
                ...
                <DialogFooter className="flex sm:justify-between">
                    <div className="text-sm text-muted-foreground">
                        ...
                        {assetsNotInGroupQuery.data?.total || 0} selected
                    </div>
                    <Button
                        type="submit"
