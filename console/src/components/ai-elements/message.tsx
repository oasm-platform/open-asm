  ...
  ({ className, ...props }: MessageResponseProps) => {
    useEffect(() => {
      ...
      const handleGlobalCopy = (e: MouseEvent) => {
        const btn = (e.target as Element).closest('[data-streamdown="code-block-copy-button"]');
        if (btn && (!navigator?.clipboard?.writeText)) {
          const codeBlock = btn.closest('[data-streamdown="code-block"]')?.querySelector('code') || btn.closest('.group')?.querySelector('code') || btn.closest('.relative')?.querySelector('code');
          if (codeBlock) {
